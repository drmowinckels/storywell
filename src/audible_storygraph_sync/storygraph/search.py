...
NOTE: ``SEARCH_URL``, ``RESULT_SELECTOR`` and ``_EXTRACT_JS`` are provisional and
must be verified against the live StoryGraph DOM (needs an authenticated session).
The pure helpers (``parse_book_id``, ``_candidates_from_records``) are version-stable
and unit-tested; only the selectors are expected to need adjustment.
"""
...
RESULT_SELECTOR = "[data-test-id='book-pane'], .book-pane, .search-results-books-panes > div"
_BOOK_ID_RE = re.compile(r"/books/([^/?#]+)")

_EXTRACT_JS = """
(el) => {
  const link = el.querySelector("a[href*='/books/']");
  const titleEl = el.querySelector(".book-title, [data-test-id='book-title'], h3, h2");
  const authorEl = el.querySelector(".book-author, [data-test-id='book-author'], .author");
  return {
...
class StorygraphSearcher:
    ...
    def search(self, query: str) -> list[Candidate]:
        self._page.goto(_search_url(query))
